...
export default function CartItem({ meals, categories }) {
  ...
  function formatHours(startHour, endHour) {
    ...
    const formattedStartHour = pad(startHour) + ":00";
    const formattedEndHour = pad(endHour) + ":00";
  ...
  return (
    <>
      {(result.isActive) ? <Suspense>
        <NavLink
          to="/restaurants"
          className="absolute w-[30px] h-[30px] text-white left-[10px] top-[10px]"
        >
          {" "}
          <FontAwesomeIcon icon={faArrowLeft} className="w-full h-full outline-black" />
        </NavLink>
        ...
        >
          {(result.canOrder != null ? result.canOrder : true) && (
            <div
              ...
              onClick={() => {
                if (isBasketOpen) setIsOpen(true);
              }}
            >
          ...
          <div className="infoPart">
            ...
            <div className="UpperPart">
              ...
              >
                {result.name}
                <span>{result.serviceSector ? result.serviceSector[language] : ""}</span>
              </span>
              <AiOutlineHeart />
            </div>
              ...
              <div>
                ...
                <span>
                  {result.startHour && result.endHour
                    ? formatHours(result.startHour, result.endHour)
                    : ""}
                </span>
              </div>
            </div>
        ...
        <div className="p-2 flex flex-col items-center text-center">
          ...
          <div className="flex items-center">
            <p className="text-[1.2em]">
              Hörmətli müştəri,<br />
              Sizin Qr-menyuazın abunə vaxtı bitdiyi üçün sisteminiz bağlanmışdır. Ətraflı məlumat üçün zəhmət olmasa bizimlə əlaqə saxlayın.<br />
              Tel:055-801-03-04
            </p>
          </div>
